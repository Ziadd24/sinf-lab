/**
 * SQLite-compatible testIds serialization.
 *
 * SQLite doesn't support native arrays (String[]), so we store testIds
...
export function serializeTestIds(ids: string[]): string {
  return JSON.stringify(ids);
}

/** Convert JSON string from DB → string[] for app use */
export function deserializeTestIds(raw: string): string[] {
  if (!raw) return [];
  try {
...
export function deserializeSample<T extends { testIds: string }>(sample: T): Omit<T, "testIds"> & { testIds: string[] } {
  return {
    ...sample,
    testIds: deserializeTestIds(sample.testIds),
  };
}